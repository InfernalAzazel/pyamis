...
class ActionType:
    ...
    class Ajax(Action):
        ...
        redirect: Optional[
            Template] = None  # Specify the path to redirect to after the current request ends, which can be
        # valued by ${xxx}.
        ...
        messages: Optional[
            dict] = None  # success: a message will be displayed after the ajax operation is successful. It can
    ...
    class Copy(Action):
        ...
        copyFormat: Optional[
            str] = None  # You can set the copy format through copyFormat, the default is text text/html
...
class PageSchema(AmisNode):
    ...
    url: Optional[
        str] = None  # The page routing path, when the route hits the path, the current page is enabled. When the
    ...
    rewrite: Optional[
        str] = None  # Change to rendering pages of other paths, the page address will not be modified in this way.
    isDefaultPage: Union[
        bool, str, None] = None  # Useful when you need a custom 404 page, don't have multiple such pages,
    ...
    tabsMode: Optional[
        TabsModeEnum] = None  # Unofficial attribute. Display mode, the value can be line, card, radio, vertical,
    ...
    def as_page_body(self, group_extra: Optional[Dict[str, Any]] = None, item_extra: Optional[Dict[str, Any]] = None):
        if self.children:
            exclude = {"type", "url", "schema_", "schemaApi", "link", "redirect", "rewrite", "isDefaultPage",
                       "children"}
            if self.tabsMode is None:
                ...
                body = (
                    ...
                    .update_from_kwargs(
                        body=[
                            Collapse.parse_obj(item.dict(exclude=exclude, exclude_defaults=True))
                            .update_from_kwargs(
                                header=item.label,
...
class Service(AmisNode):
    ...
    messages: Optional[
        dict] = None  # Message prompt override, the default message reads the toast prompt text returned by the
...
class AnchorNav(AmisNode):
    ...
    direction: Optional[
        str] = None  # "vertical" # You can configure whether the navigation is displayed horizontally or
...
class InputSubForm(FormItem):
    ...
    labelField: Optional[
        str] = None  # When this field exists in the value, the button name will be displayed using the value
    # of this field.
...
class Button(FormItem):
    ...
    url: Optional[
        str] = None  # Click the jump address, specify the behavior of this attribute button is consistent with the
    # a link
    ...
    level: Optional[
        LevelEnum] = None  # Set button style 'link'|'primary'|'enhance'|'secondary'|'info'|'success'|'warning
...
class InputFormula(FormItem):
    ...
    variables: Optional[
        List[dict]] = None  # Available variables, {label: string; value: string; children?: any[]; tag?: string}
...
class InputArray(FormItem):
    ...
    removable: Optional[bool] = None  # whether it can be removed
    draggable: Optional[
        bool] = None  # False # whether drag sorting is possible, it should be noted that when drag sorting is
    # enabled, there will be an additional $id field
    draggableTip: Optional[
        str] = None  # Draggable prompt text, the default is: "Order can be adjusted by dragging the [Swap]
    # button in each row"
...
class InputCity(FormItem):
    ...
    extractValue: Optional[
        bool] = None  # True# whether to extract the value, if set to false, the value format will become an
...
class InputColor(FormItem):
    ...
    presetColors: Optional[
        List[str]] = None  # "Selector preset color value" # The default color at the bottom of the selector,
    # if the array is empty, the default color will not be displayed
    allowCustomColor: Optional[
        bool] = None  # True # When false, only colors can be selected, use presetColors to set the
...
class Combo(FormItem):
    ...
    deleteApi: Optional[
        API] = None  # If configured, an api will be sent before deletion, and the deletion will be completed
    # after the request is successful
    deleteConfirmText: Optional[
        str] = None  # "Confirm to delete?" # It only takes effect when deleteApi is configured! Used
    ...
    # enabled, there will be an additional $id field
    draggableTip: Optional[
        str] = None  # "Order can be adjusted by dragging the [Exchange] button in each row" # Draggable
    # prompt text
    ...
    # to the form item
    conditions: Optional[
        dict] = None  # The form of an array contains the rendering types of all conditions. The test in a
    ...
    subFormHorizontal: Optional[
        dict] = None  # The horizontal configuration of the sub-form item, which is the same as the
...
class Editor(FormItem):
    ...
    type: str = "editor"
    language: Optional[
        str] = None  # "javascript" # The language highlighted by the editor, which can be obtained through the
    # ${xxx} variable
    ...
    options: Optional[
        dict] = None  # Other configurations of the monaco editor, such as whether to display line numbers, etc.,
...
class Formula(AmisNode):
    ...
    id: Optional[
        bool] = None  # Default True, Define a name. When a button's target is specified, it will trigger a formula.
...
class GridNav(AmisNode):
    ...
    class OptionsItem(AmisNode):
        ...
        link: Optional[
            str] = None  # default '', Internal page path or external URL address, takes precedence over clickAction
...
class InputFile(FormItem):
    ...
    maxSize: Optional[
        int] = None  # There is no limit by default, when set, the file size larger than this value will not be
    ...
    stateTextMap: Optional[
        dict] = None  # Upload state text Default: {init: '', pending: 'Waiting to upload', uploading:
    ...
    autoFill: Optional[
        Dict[str, str]] = None  # After the upload is successful, the value returned by the upload interface can
...
class InputTable(FormItem):
    ...
    needConfirm: Optional[
        bool] = None  # True # whether to confirm the operation, it can be used to control the operation
    # interaction of the control table
    canAccessSuperData: Optional[
        bool] = None  # False # whether you can access the parent data, that is, the same level data
    # in the form, usually need to be used with strictMode
    strictMode: Optional[
        bool] = None  # True # For performance, the default value of other form items will not update the
...
class InputImage(FormItem):
    ...
    class CropInfo(BaseAmisModel):
        aspectRatio: Optional[
            float] = None  # Crop ratio. Floating point, the default is 1, which is 1:1. If you want to set
    ...
    class Limit(BaseAmisModel):
        ...
        aspectRatio: Optional[
            float] = None  # Limit the aspect ratio of the image, the format is a floating-point number,
    ...
    fixedSize: Optional[
        bool] = None  # whether to enable fixed size, if enabled, set fixedSizeClassName at the same time
...
class Picker(FormItem):
    ...
    autoFill: Optional[dict] = None  # autofill
    modalMode: Optional[
        Literal["dialog", "drawer"]] = None  # "dialog" # Set dialog or drawer to configure the popup mode.
    pickerSchema: SerializeAsAny[
        Optional[Union[SchemaNode, "CRUD"]]] = None  # "{mode: 'list', listItem: {title: '${label}'}}"
...
class InputText(FormItem):
    ...
    addOn: SerializeAsAny[
        Optional[SchemaNode]] = None  # Input box add-ons, such as with a prompt text, or with a submit button.
    trimContents: Optional[bool] = None  # whether to remove leading and trailing blank text.
    creatable: Optional[
        bool] = None  # whether it can be created, the default is yes, unless it is set to false, only the
...
class InputRichText(FormItem):
    ...
    buttons: Optional[
        List[str]] = None  # froala dedicated, configure the displayed buttons, tinymce can set the toolbar
...
class InputRating(FormItem):
    ...
    colors: Union[
        str, dict, None] = None  # default {'2': '#abadb1', '3': '#787b81', '5': '#ffa900' }, The color in which
...
class InputRange(FormItem):
    ...
    clearable: Optional[
        bool] = None  # default False, whether the precondition can be cleared : showInputValid when enabled
...
class Timeline(AmisNode):
    ...
    source: Optional[
        API] = None  # Data source, you can obtain current variables through data mapping, or configure API objects
...
class TooltipWrapper(AmisNode):
    ...
    offset: Optional[
        Tuple[int, int]] = None  # default [0, 0], relative offset of the position of the text prompt, in px
...
class InputTag(FormItem):
    ...
    overflowTagPopover: Optional[
        TooltipWrapper] = None  # default {"placement": "top", "trigger": "hover", "showArrow": false,
    ...
    separator: Optional[
        str] = None  # default "-", After batch adding is enabled, enter the delimiter of multiple labels,
...
class Select(FormItem):
    ...
    checkAllBySearch: Optional[
        bool] = None  # False # When there is a search, only all items hit by the search are selected
    defaultCheckAll: Optional[bool] = None  # False # whether to check all by default
    creatable: Optional[bool] = None  # False # New option
    ...
    selectMode: Optional[
        str] = None  # Optional: group, table, tree, chained, associated. They are: list form, table form,
    ...
    searchResultMode: Optional[
        str] = None  # If the value of selectMode is not set, it can be configured separately. Refer to
    # selectMode to determine the display form of search results.
    columns: Optional[
        List[dict]] = None  # When the display form is table, it can be used to configure which columns are
    # displayed, which is similar to the columns configuration in table, but only has the display function.
    leftOptions: Optional[
        List[dict]] = None  # Used to configure the left option set when the display form is associated.
    leftMode: Optional[
        str] = None  # When the display form is associated, it is used to configure the left selection form,
    # support list or tree. Default is list.
    rightMode: Optional[
        str] = None  # When the display form is associated, it is used to configure the right selection form,
...
class NestedSelect(Select):
    ...
    cascade: Optional[
        bool] = None  # False # When set to true, child nodes are not automatically selected when the parent node
    # is selected.
    withChildren: Optional[
        bool] = None  # False # When set to true, when the parent node is selected, the value of the child
    # node will be included in the value, otherwise only the value of the parent node will be retained.
    onlyChildren: Optional[
        bool] = None  # False # For multiple selections, whether to add only its child nodes to the value
    ...
    hideNodePathLabel: Optional[
        bool] = None  # False # whether to hide the path label information of the selected node in the
    # selection box
...
class Card(AmisNode):
    ...
    class Header(AmisNode):
        ...
        avatarClassName: Optional[
            str] = None  # default "pull-left thumb avatar b-3x m-r", Image includes layer class name
...
class InputMonth(FormItem):
    ...
    inputFormat: Optional[
        str] = None  # "YYYY-MM" # The display format of the month selector, that is, the timestamp format.
...
class InputTime(FormItem):
    ...
    value: Optional[str] = None  # default value
    timeFormat: Optional[
        str] = None  # "HH:mm" # Time selector value format, please refer to moment for more format types
    format: Optional[str] = None  # "X" # Time selector value format, please refer to moment for more format types
    inputFormat: Optional[
        str] = None  # "HH:mm" # Time selector display format, that is, timestamp format, please refer to
...
class InputDatetime(FormItem):
    ...
    value: Optional[str] = None  # default value
    format: Optional[
        str] = None  # "X" # Date time picker value format, please refer to the documentation for more format types
    inputFormat: Optional[
        str] = None  # "YYYY-MM-DD HH:mm:ss" # Date time picker display format, namely timestamp format,
...
class InputDate(FormItem):
    ...
    value: Optional[str] = None  # default value
    format: Optional[
        str] = None  # "X" # Date picker value format, please refer to the documentation for more format types
    inputFormat: Optional[
        str] = None  # "YYYY-DD-MM" # Date picker display format, that is, timestamp format, please refer to
    ...
    closeOnSelect: Optional[
        bool] = None  # False # whether to close the selection box immediately after clicking the date
    schedules: Union[
        list, str, None] = None  # The schedule is displayed in the calendar, static data can be set or data
...
class Calendar(FormItem):
    ...
    scheduleClassNames: Optional[
        List[str]] = None  # default ['bg-warning', 'bg-danger', 'bg-success', 'bg-info', 'bg-secondary']
...
class InputDatetimeRange(InputTimeRange):
    ...
    ranges: Union[
        str, List[str], None] = None  # "yesterday,7daysago,prevweek,thismonth,prevmonth,prevquarter" date range
...
class InputTree(FormItem):
    ...
    unfoldedLevel: Optional[
        int] = None  # 0 # Set the default unfolded level, which only takes effect when initiallyOpen is
    # not true.
    cascade: Optional[bool] = None  # False # Do not automatically select child nodes when parent node is selected.
    withChildren: Optional[
        bool] = None  # False # When the parent node is selected, the value will contain the value of the
    ...
    pathSeparator: Optional[
        str] = None  # "/" # The separator of the node path, it takes effect when enableNodePath is true
    deferApi: Optional[
        API] = None  # For lazy loading options, please configure defer to true, and then configure deferApi to
...
class Image(AmisNode):
    ...
    imageMode: Optional[
        str] = None  # "thumb" Image display mode, optional: 'thumb', 'original' ie: thumbnail mode or original
    # image mode
...
class Images(AmisNode):
    ...
    originalSrc: Optional[
        str] = None  # Original image address, support data mapping to obtain image variables in the object
    enlargeAble: Optional[bool] = None  # Support zoom in preview
    thumbMode: Optional[str] = None  # "contain" # preview mode, optional: 'w-full','h-full','contain','cover'
    thumbRatio: Optional[str] = None  # "1:1" # Preview ratio, optional: '1:1','4:3','16:9'
...
class Carousel(AmisNode):
    ...
    controlsTheme: Optional[
        str] = None  # "light" # Left and right arrows, bottom dot index color, default light, and dark mode
...
class CRUD(AmisNode):
    ...
    loadDataOnceFetchOnFilter: Optional[
        bool] = None  # True # When loadDataOnce is turned on, whether to re-request the api
    # when filtering
    source: Optional[
        str] = None  # The data mapping interface returns the value of a field. If it is not set, the ${items} or
    ...
    initFetch: Optional[
        bool] = None  # True # whether to pull data during initialization, only for the case with filter,
    ...
    stopAutoRefreshWhen: Optional[str] = None  # Configure the condition for stopping refresh by expression
    stopAutoRefreshWhenModalIsOpen: Optional[
        bool] = None  # Turn off auto refresh when there is a popup, close the popup and
    # restore
    ...
    perPageField: Optional[
        str] = None  # "perPage" # Set the field name of how many pieces of data are displayed on one page.
    # Note: Best used with defaultParams, see example below.
    perPageAvailable: Optional[
        List[int]] = None  # [5, 10, 20, 50, 100] # Set how many data drop-down boxes can be displayed
    # on one page.
    orderField: Optional[
        str] = None  # Set the field name used to determine the position. After setting, the new order will be
    ...
    autoGenerateFilter: Optional[
        bool] = None  # whether to open the query area, after it is enabled, the query condition form
    # will be automatically generated according to the searchable attribute value of the column element
    itemAction: Optional[
        Action] = None  # Implement custom actions after clicking a row, support all configurations in action,
...
class Log(AmisNode):
    ...
    type: str = "log"
    source: Optional[
        API] = None  # Support variables, which can be initially set to empty, so that it will not be loaded
...
class Progress(AmisNode):
    ...
    gapDegree: Optional[
        int] = None  # default 75, angle of the missing corner of the instrument panel, the value can be 0 ~ 295
    gapPosition: Optional[
        str] = None  # default "bottom", Dashboard progress bar notch position, optional top bottom left right
...
class Video(AmisNode):
    ...
    isLive: Optional[
        bool] = None  # False # whether it is a live broadcast, it needs to be added when the video is live,
...
class Dialog(AmisNode):
    ...
    showErrorMsg: Optional[
        bool] = None  # True # whether to display the error message in the lower left corner of the popup
    disabled: Optional[
        bool] = None  # False # If this property is set, the Dialog is read-only and has no submit operation.
    actions: SerializeAsAny[
        Optional[List[Action]]
    ] = None  # If you want to not display the bottom button, you can configure: [] "[Confirm]
    # and [Cancel]"
    data: Optional[
        dict] = None  # Support data mapping, if not set, it will inherit the data in the context of the trigger
    # button by default.
...
class Tasks(AmisNode):
    ...
    interval: Optional[
        int] = None  # 3000 # When there is a task in progress, it will be checked again at regular intervals,
    ...
    statusTextMap: Optional[
        List[str]] = None  # "["Not started", "Ready", "In progress", "Error", "Completed", "Error"]" #
...
class Wizard(AmisNode):
    ...
    class Step(AmisNode):
        ...
        mode: Optional[
            str] = None  # Display the default, the same as the mode in Form, choose: normal, horizontal or inline.
        horizontal: Optional[
            Horizontal] = None  # When in horizontal mode, it is used to control the ratio of left and right
        ...
        initFetchOn: Optional[
            Expression] = None  # whether the current step data initialization interface is initially fetched
    ...
    target: Optional[
        str] = None  # "False" # You can submit data to other components instead of saving it yourself. Please
    ...
    startStep: Optional[
        int] = None  # "1" # Start default value, starting from the first step. Templates can be supported,
